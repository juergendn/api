...
ProtoBuf contains a commandline program for easy testing of service endpoints:
[buf curl](https://buf.build/docs/reference/cli/buf/curl/)

It allows sending protoBuf messages from a commandline via JSON.\
All example workflows in the following sections are given as "buf curl" JSON strings.

The `buf curl` in this command uses `--data @-` as a parameter,
which will keep the connection open and accepts JSON input via `stdin`.

Invocation preceding each example:

    buf curl --schema public/proto --protocol grpc --http2-prior-knowledge http://10.10.99.34:11111/local.v1.PayService/Pay --data @-
